...
class ObservabilityCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        self.llm_calls.append({"duration": duration, "tokens": token_usage})
        ...
        track_llm_call(
            ...
            success=True,
            duration=duration,
            tokens=tokens_dict if tokens_dict else None,
        )
    ...
    def on_tool_start(
        ...
        self.tool_calls.append(
            {"tool": tool_name, "input": input_str, "start_time": time.time()}
        )
    ...
    def on_tool_end(self, output: str, **kwargs: Any) -> None:
        if self.tool_calls:
            ...
            success = not (
                output and (output.startswith("Error") or output.startswith("Invalid"))
            )
...
def validate_url(url: str) -> bool:
    ...
    try:
        ...
        if parsed.hostname in ["localhost", "127.0.0.1"] or parsed.hostname.startswith(
            "192.168."
        ):
            return False
        return True
...
def fetch_url_content(url: str) -> str:
    ...
    def _fetch():
        ...
        for i, user_agent in enumerate(user_agents):
            try:
                ...
                if response.status_code == 403:
                    logger.warning(
                        f"Got 403 for {url} with User-Agent {i+1}/{len(user_agents)}, trying next...",
                ...
                if e.response and e.response.status_code == 403:
                    logger.warning(
                        f"HTTP 403 error for {url} with User-Agent {i+1}/{len(user_agents)}, trying next...",
                        extra={"trace_id": trace_id},
                    )
                    continue
                else:
        ...
        max_chars = (
            MAX_CONTENT_SIZE * 10
        )  # Allow 500KB of markdown text for large pages
        ...
        if len(markdown_content) > max_chars:
            ...
            if (
                last_paragraph > max_chars * 0.8
            ):  # If we can find a paragraph break in last 20%
                markdown_content = (
                    truncated[:last_paragraph]
...
def extract_source_from_question(
    question: str, available_sources: List[Dict[str, Any]]
) -> Optional[str]:
    ...
    if urls:
        ...
        for url in urls:
            ...
            for doc in available_sources:
                ...
                if (
                    normalized_url == doc_source
                    or normalized_url in doc_source
                    or doc_source in normalized_url
                ):
    ...
    for doc in available_sources:
        ...
        try:
            ...
            source_clean = (
                source.replace("https://", "").replace("http://", "").rstrip("/")
            )
            if source_clean in question_lower or any(
                part in question_lower
                for part in source_clean.split(".")
                if len(part) > 2
            ):
                return doc.get("source")
    ...
    try:
        with driver.session() as session:
            ...
            if source_filter:
                ...
                """
                result = session.run(
                    search_query, source_filter=source_filter, limit=limit
                )
            else:
    ...
    try:
        with driver.session() as session:
            ...
            if not available_docs:
                track_neo4j_operation(
                    "search", success=False, duration=time.time() - search_start
                )
            ...
            matched_source = extract_source_from_question(
                filtered_question, available_docs
            )
            ...
            chunks_for_llm = []
            for idx, chunk_info in enumerate(
                all_chunks[:150]
            ):  # Limit to 150 chunks for LLM processing
...
If no chunks are relevant, return an empty array: []"""
            ...
            filter_messages = [
                SystemMessage(
                    content="You are a semantic relevance filter. Understand questions semantically and select relevant chunks. Return only a JSON array of chunk indices."
                ),
            ...
            tokens_dict = None
            if (
                hasattr(filter_response, "response_metadata")
                and filter_response.response_metadata
            ):
            ...
            try:
                ...
                if "```json" in filter_clean:
                    filter_clean = (
                        filter_clean.split("```json")[1].split("```")[0].strip()
                    )
                ...
                logger.warning(
                    f"Failed to parse LLM filter response: {filter_result[:200]}, using all chunks",
                    extra={"trace_id": trace_id},
                )
                chunks_with_source = all_chunks[:limit]
            ...
            for chunk_info in chunks_with_source:
                source_info = (
                    f"[Source: {chunk_info['source']}, Type: {chunk_info['doc_type']}]"
                )
                result_parts.append(f"{source_info}\n{chunk_info['text']}")
            result_text = "\n\n---\n\n".join(result_parts)
            track_neo4j_operation(
                "search", success=True, duration=time.time() - search_start
            )
            return result_text
...
def store_document(source: str, content: str, doc_type: str = "webpage") -> str:
    ...
    try:
        ...
        # Split into chunks
        chunks = (
            text_splitter.split_text(filtered_content)
        ...
        if not chunks:
            if filtered_content and filtered_content.strip():
                ...
                logger.warning(
                    f"No content to chunk after filtering. Original content length: {len(content) if content else 0}",
                    extra={"trace_id": trace_id},
                )
                chunks = []
        ...
        if not chunks:
            if (
                filtered_content
                and filtered_content.strip()
                and len(filtered_content.strip()) >= 10
            ):
                logger.warning(
                    "All chunks filtered out, using full filtered content as single chunk",
                    extra={"trace_id": trace_id},
                )
                chunks = [filtered_content.strip()]
            elif content and content.strip() and len(content.strip()) >= 10:
                logger.warning(
                    "Using original content as fallback (filtered content was empty)",
                    extra={"trace_id": trace_id},
                )
                chunks = [content.strip()]
            else:
                logger.error(
                    f"No valid content to store. Filtered: {len(filtered_content) if filtered_content else 0} chars, Original: {len(content) if content else 0} chars",
                    extra={"trace_id": trace_id},
                )
                chunks = []
        ...
        with driver.session() as session:
            with session.begin_transaction() as tx:
                ...
                track_neo4j_operation(
                    "create_document", success=True, duration=time.time() - doc_start
                )

                # Create chunks in batch for better performance
                if not chunks:
                    logger.error(
                        f"No chunks to store for document {source}. Document created but no chunks.",
                        extra={"trace_id": trace_id},
                    )
                ...
                if not chunk_data:
                    logger.warning(
                        f"No valid chunks to store after filtering",
                        extra={"trace_id": trace_id},
                    )
                ...
                try:
                    ...
                    record = result.single()
                    saved_chunks = (
                        record["created_count"] if record else len(chunk_data)
                    )
                    track_neo4j_operation(
                        "create_chunk", success=True, duration=time.time() - chunk_start
                    )
                    ...
                    logger.error(
                        f"Error creating chunks in batch: {chunk_error}",
                        extra={"trace_id": trace_id},
                        exc_info=True,
                    )
                    raise
        ...
        track_neo4j_operation(
            "store_document",
            success=saved_chunks > 0,
            duration=time.time() - neo4j_start,
        )

        if saved_chunks == 0:
            logger.error(
                f"No chunks were saved for document {source} despite having {len(chunks)} chunks to process",
                extra={"trace_id": trace_id},
            )
            return f"Error: No chunks were saved. Please check logs for details."

        logger.info(
            f"Document stored: {source}. ({saved_chunks}/{len(chunks)} chunks saved)",
            extra={"trace_id": trace_id},
        )
...
def determine_user_intent(user_input: str, trace_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if json_start >= 0 and json_end > json_start:
            ...
            return {
                ...
                "reasoning": intent_result.get(
                    "reasoning", "Determined by coordinator agent"
                ),
            }
        else:
        ...
        logger.warning(
            f"Failed to parse coordinator response as JSON: {output[:200] if 'output' in locals() else 'N/A'}",
            extra={"trace_id": trace_id},
        )
...
async def add_trace_id(request: Request, call_next):
    ...
    try:
        ...
        endpoint = request.url.path
        track_http_request(
            method=method, endpoint=endpoint, status=500, duration=duration
        )
        logger.error(
...
def extract_text_from_pdf_bytes(data: bytes) -> str:
            ...
            with pdfplumber.open(io.BytesIO(data)) as pdf:
                ...
                for i, page in enumerate(pdf.pages):
                    try:
                        ...
                        logger.warning(
                            f"Error extracting text from page {i + 1}: {page_error}",
                            extra={"trace_id": trace_id},
                        )
                        continue
                ...
                if text_parts:
                    ...
                    logger.warning(
                        "pdfplumber extracted no text, trying PyPDF2 fallback",
                        extra={"trace_id": trace_id},
                    )
        except Exception as e:
            logger.warning(
                f"pdfplumber failed: {e}, trying PyPDF2 fallback",
                extra={"trace_id": trace_id},
            )
    ...
    if PyPDF2:
        try:
            ...
            logger.error(
                f"PyPDF2 fallback also failed: {e}",
                extra={"trace_id": trace_id},
                exc_info=True,
            )
...
def create_fulltext_index():
    ...
    cypher = (
        "CALL db.index.fulltext.createNodeIndex('chunks_text_idx', ['Chunk'], ['text'])"
    )
    with driver.session() as session:
        try:
            session.run(cypher)
...
def ingest_url(payload: IngestURL, request: Request):
    ...
    try:
        ...
        def run_agent():
            ...
            try:
                agent_result = url_agent.invoke(
                    {"input": f"Fetch and store this URL: {payload.url}"}
                )
            except Exception as e:
                agent_error = e
        ...
        if agent_thread.is_alive():
            ...
            logger.warning(
                f"URL agent timed out after 90s, using direct tool calls",
                extra={"trace_id": trace_id},
            )
        elif agent_error:
            ...
            track_agent_invocation("url_agent", success=False, duration=agent_duration)
            logger.warning(
                f"URL agent error: {type(agent_error).__name__}, using direct tool calls",
                extra={"trace_id": trace_id},
            )
        elif agent_result:
            # Agent completed - check result
            output = (
                agent_result.get("output", str(agent_result))
            ...
            if (
                output
                and not output.strip().startswith("Error")
                and len(output.strip()) > 10
            ):
                # Agent succeeded
                track_agent_invocation(
                    "url_agent", success=True, duration=agent_duration
                )
                logger.info(
                    f"URL processed successfully by url_agent",
                    extra={"trace_id": trace_id},
                )
            else:
                # Agent returned error or empty - use fallback
                track_agent_invocation(
                    "url_agent", success=False, duration=agent_duration
                )
                logger.warning(
                    f"URL agent returned error or empty output, using direct tool calls",
                    extra={"trace_id": trace_id},
                )
                output = None
        else:
            ...
            track_agent_invocation("url_agent", success=False, duration=agent_duration)
            logger.warning(
                f"URL agent returned no result, using direct tool calls",
                extra={"trace_id": trace_id},
            )
        ...
        if not output:
            logger.debug(
                f"Processing URL using direct tool calls (fallback)",
                extra={"trace_id": trace_id},
            )
            ...
            track_tool_call(
                "fetch_url_content",
                success=not (
                    content.startswith("Error") or content.startswith("Invalid")
                ),
            ...
            if content.startswith("Error") or content.startswith("Invalid"):
                logger.error(
                    f"Failed to fetch URL content: {content[:200]}",
                    extra={"trace_id": trace_id},
                )
            ...
            if not content or len(content.strip()) < 50:
                logger.error(
                    f"Content too short or empty: {len(content) if content else 0} chars",
                    extra={"trace_id": trace_id},
                )
                return {
                    "status": "error",
            ...
            result = store_document.invoke(
                {"source": payload.url, "content": content, "doc_type": "webpage"}
            )
            track_tool_call(
                "store_document",
        ...
        if output.startswith("Error"):
            logger.error(
                f"Failed to process URL: {output[:200]}", extra={"trace_id": trace_id}
            )
        ...
        with driver.session() as session:
            ...
            if chunk_count == 0:
                return {
                    "status": "error",
                    "message": "URL processed but no chunks were saved to database",
                }

        logger.info(
            f"Successfully ingested URL: {payload.url}, created {chunk_count} chunks",
            extra={"trace_id": trace_id},
        )
        return {"status": "ok", "source": payload.url, "message": output}
    except Exception as e:
        logger.error(
            f"Ingest URL error: {e}", extra={"trace_id": trace_id}, exc_info=True
        )
...
def ingest_file(file: UploadFile = File(...), request: Request = None):
    ...
    trace_id = (
        getattr(request.state, "trace_id", trace_context.get_trace_id())
        if request
        else trace_context.get_trace_id()
    )
    trace_context.set_trace_id(trace_id)

    try:
        logger.info(
            f"Ingesting file: {file.filename}, content_type: {file.content_type}",
            extra={"trace_id": trace_id},
        )
        ...
        if not text or not text.strip():
            return {
                "status": "error",
                "message": "Не удалось извлечь текст из файла. Возможно, файл пуст, зашифрован или содержит только изображения.",
            }

        # Store using tool
        result = store_document.invoke(
            {"source": filename, "content": text, "doc_type": doc_type}
        )

        logger.info(
            f"File ingested successfully: {filename}", extra={"trace_id": trace_id}
        )
        return {"status": "ok", "source": filename, "message": result}
    except Exception as e:
        logger.error(
            f"Ingest file error: {e}", extra={"trace_id": trace_id}, exc_info=True
        )
        ...
        if intent == "SAVE_URL":
            ...
            if url:
                ...
                try:
                    ...
                    if content.startswith("Error") or content.startswith("Invalid"):
                        return {
                            "answer": f"Ошибка при загрузке страницы: {content[:200]}"
                        }
                    ...
                    tool_start = time.time()
                    result = store_document.invoke(
                        {"source": url, "content": content, "doc_type": "webpage"}
                    )
                    track_tool_call(
                        "store_document",
                        success=not result.startswith("Error"),
                    ...
                    if (
                        "successfully" in filtered_output.lower()
                        or "stored" in filtered_output.lower()
                    ):
                        return {"answer": "Страница успешно сохранена в базу знаний."}
                    else:
                        return {
                            "answer": (
                                filtered_output if is_safe else "Страница сохранена."
                            )
                        }
                except Exception as e:
                    logger.error(
                        f"Error saving URL: {e}",
                        extra={"trace_id": trace_id},
                        exc_info=True,
                    )
                ...
                return {
                    "answer": "Не удалось извлечь URL из команды. Пожалуйста, укажите URL явно."
                }
            ...
            try:
                ...
                try:
                    ...
                    if (
                        answer
                        and not answer.strip().startswith("Error")
                        and len(answer.strip()) > 10
                    ):
                        # Agent succeeded
                        track_agent_invocation(
                            "search_agent", success=True, duration=agent_duration
                        )
                        logger.info(
                            f"Query processed successfully by search_agent",
                            extra={"trace_id": trace_id},
                        )
                    else:
                        # Agent returned error or empty - use fallback
                        track_agent_invocation(
                            "search_agent", success=False, duration=agent_duration
                        )
                        logger.warning(
                            f"Search agent returned error or empty output, using direct tool calls",
                            extra={"trace_id": trace_id},
                        )
                        answer = None
                    ...
                    logger.warning(
                        f"Search agent error: {type(agent_error).__name__}, using direct tool calls",
                        extra={"trace_id": trace_id},
                    )
                    answer = None
                ...
                if not answer:
                    logger.debug(
                        f"Processing query using direct tool calls (fallback)",
                        extra={"trace_id": trace_id},
                    )
                    tool_start = time.time()
                    context = search_graph_rag.invoke(
                        {"question": query_text, "limit": 20}
                    )
                    track_tool_call(
                        "search_graph_rag",
                        success=not (context.startswith("Error") if context else False),
                        duration=time.time() - tool_start,
                    )
                    if (
                        context
                        and not context.startswith("Error")
                        and len(context.strip()) > 10
                    ):
                        ...
                        is_detailed = any(
                            keyword in question_lower for keyword in detail_keywords
                        )

                        # For questions about "this article" or "эта статья", provide comprehensive answer
                        if (
                            "эта статья" in question_lower
                            or "this article" in question_lower
                            or "о чем" in question_lower
                        ):
                            is_detailed = True
...
Provide a clear and concise answer. Focus on key information. Use 1-2 paragraphs maximum. Be brief and to the point. Use Russian if question is in Russian, else English. Use plain text only - no markdown formatting."""
                        ...
                        answer = (
                            context
                            if context
                            else "No information found in knowledge base."
                        )
                ...
                logger.error(
                    f"Search error: {e}", extra={"trace_id": trace_id}, exc_info=True
                )
                raise

        else:  # UNKNOWN
            return {
                "answer": "Не понял ваш запрос. Вы можете отправить ссылку для сохранения или задать вопрос по сохраненным данным."
            }
        ...
        lang = (
            "ru"
            if any(c in q.question.lower() for c in "абвгдеёжзийклмнопрстуфхцчшщъыьэюя")
            else "en"
        )
        error_msg = (
            f"Ошибка при обработке запроса: {str(e)[:200]}"
            if lang == "ru"
            else f"Error processing query: {str(e)[:200]}"
        )
...
def query_time(tq: TimeQuery, request: Request):
    ...
    try:
        logger.info(
            f"Processing time query: {tq.question[:100]}...",
            extra={"trace_id": trace_id},
        )
        ...
        try:
            ...
            context = search_graph_rag.invoke(
                {"question": question_with_time, "limit": 20}
            )
            if context and not context.startswith("Error"):
                ...
                messages = [
                    SystemMessage(content=system_prompt),
                    HumanMessage(
                        content=f"Question: {question_with_time}\n\nContext:\n{context[:4000]}"
                    ),
                ]
                # Create callback for LLM metrics
                callback = ObservabilityCallbackHandler(trace_id)
        ...
        logger.error(
            f"Query_time error: {e}", extra={"trace_id": trace_id}, exc_info=True
        )
        return {
            "status": "error",
