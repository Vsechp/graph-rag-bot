...
telegram_errors_total = Counter(
    ...
    'Total number of Telegram bot errors',
    ['error_type']
)
...
tool_calls_total = Counter(
    ...
    ['tool_name', 'status']  # fetch_url_content, store_document, search_graph_rag, success/failure
)
...
neo4j_documents_count = Gauge(
    ...
    'Number of documents in Neo4j database'
)

# Количество chunks в базе
neo4j_chunks_count = Gauge(
    'neo4j_chunks_count',
    'Number of chunks in Neo4j database'
)
...
neo4j_operations_total = Counter(
    ...
    ['operation_type', 'status']  # create_document, create_chunk, search, success/failure
)

# Время выполнения операций Neo4j
neo4j_duration_seconds = Histogram(
    'neo4j_duration_seconds',
    'Neo4j operation duration in seconds',
    ['operation_type']
)
...
def track_agent_invocation(agent_name: str, success: bool, duration: Optional[float] = None, iterations: Optional[int] = None):
...
def track_llm_call(model: str, success: bool, duration: float, tokens: Optional[dict] = None):
...
def track_neo4j_operation(operation_type: str, success: bool, duration: Optional[float] = None):
